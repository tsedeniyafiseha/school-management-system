...
const CustomTooltipContent = ({ active, payload, dataKey }) => {
    if (active && payload && payload.length) {
        const { subject, attendancePercentage, totalClasses, attendedClasses, marks_obtained, subName } = payload[0].payload;
        ...
        return (
            <CustomTooltip>
                {dataKey === "attendancePercentage" ? (
                    ...
                    <>
                        <TooltipMain>{subName.sub_name}</TooltipMain>
                        <TooltipText>Marks: {marks_obtained}</TooltipText>
                    </>
                )}
            </CustomTooltip>
        );
...
const CustomBarChart = ({ chartData, dataKey }) => {
    ...
    return (
        <BarChart width={500} height={500} data={chartData}>
            <XAxis dataKey={dataKey === "marks_obtained" ? "subName.sub_name" : "subject"} />
            <YAxis domain={[0, 100]} />
